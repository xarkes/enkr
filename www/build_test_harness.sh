...
# harness on demand — see `src/testkit_support.rs::launch_test_harness`,
...
# `tests/app_sync.rs`'s CdpDriver scenarios (clicking_a_note_selects_it_cdp
...
# in Cargo.toml) done once beforehand.
set -euo pipefail
cd "$(dirname "$0")/.."
...
echo "Built. cargo test --test app_sync --features cdp"
